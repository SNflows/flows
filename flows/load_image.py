...
def load_image(FILENAME, target_coord=None):
    ...
    with fits.open(FILENAME, mode='readonly') as hdul:
        ...
        if origin == 'LCOGT':
            ...
            image.photfilter = {'zs': 'zp'}.get(hdr['FILTER'], hdr['FILTER'])
            ...
            if 'FILTER' in hdr:
                image.photfilter = {'B Bes': 'B', 'V Bes': 'V', 'R Bes': 'R', 'g SDSS': 'gp', 'r SDSS': 'rp',
                                    'i SDSS': 'ip', 'i int': 'ip',  # Interference filter
                                    'u SDSS': 'up', 'z SDSS': 'zp'}.get(hdr['FILTER'].replace('_', ' '), hdr['FILTER'])
            else:
                filters_used = []
                ...
                if len(filters_used) == 1:
                    image.photfilter = {'B_Bes 440_100': 'B', 'V_Bes 530_80': 'V', 'R_Bes 650_130': 'R',
                                        "g'_SDSS 480_145": 'gp', "r'_SDSS 618_148": 'rp', "i'_SDSS 771_171": 'ip',
            ...
            for check_headers in ('NCFLTNM1', 'NCFLTNM2'):
                if hdr.get(check_headers) and hdr.get(check_headers).strip().lower() != 'open':
                    filters_used.append(hdr.get(check_headers).strip())
            if len(filters_used) == 1:
                image.photfilter = {'Ks': 'K'}.get(filters_used[0], filters_used[0])
            else:
                raise RuntimeError("Could not determine filter used.")
            ...
            image.photfilter = {'g.00000': 'gp', 'r.00000': 'rp', 'i.00000': 'ip', 'z.00000': 'zp'}.get(
                hdr['FPA.FILTER'], hdr['FPA.FILTER'])
            ...
            image.obstime = Time(hdr['DATE-OBS'], format='isot', scale='utc', location=image.site['EarthLocation'])
            image.obstime += 0.5 * image.exptime * u.second  # Make time centre of exposure
            image.photfilter = {'Bessel-B': 'B', 'Bessell-B': 'B', 'Bessel-V': 'V', 'Bessell-V': 'V', 'SDSS-U': 'up',
                                'SDSS-G': 'gp', 'SDSS-R': 'rp', 'SDSS-I': 'ip', 'SDSS-Z': 'zp'}.get(hdr['FILTER1'],
                                                                                                    hdr['FILTER1'])
            ...
            image.photfilter = {'u': 'up', 'g': 'gp', 'r': 'rp', 'i': 'ip', }.get(hdr['FILTER'], hdr['FILTER'])
            ...
            image.obstime = Time(hdr['JD'], format='jd', scale='utc', location=image.site['EarthLocation'])
            image.photfilter = {'Yc': 'Y', 'Hc': 'H', 'Jo': 'J', }.get(hdr['FILTER'], hdr['FILTER'])
            ...
            image.obstime = Time(hdr['JD'], format='jd', scale='utc', location=image.site['EarthLocation'])
            image.photfilter = {'Ks': 'K', 'J1': 'Y', }.get(hdr['FILTER'], hdr['FILTER'])
            ...
            image.obstime = Time(hdr['DATE-OBS'], format='isot', scale='utc', location=image.site['EarthLocation'])
            image.obstime += 0.5 * image.exptime * u.second  # Make time centre of exposure
            image.photfilter = {'g782': 'gp', 'r784': 'rp', 'i705': 'ip', 'B639': 'B', 'V641': 'V'}.get(hdr['FILTER'],
                                                                                                        hdr['FILTER'])
            ...
            image.photfilter = {'H_Open': 'H', 'K_Open': 'K', }.get(hdr['FILTER'], hdr['FILTER'])
            ...
            image.obstime = Time(hdr['MJD-OBS'], format='mjd', scale='utc', location=image.site['EarthLocation'])
            image.obstime += 0.5 * image.exptime * u.second  # Make time centre of exposure
            image.photfilter = {'i_SDSS': 'ip'}.get(hdr['ESO INS FILT1 NAME'], hdr['ESO INS FILT1 NAME'])
            ...
            image.photfilter = {'j': 'J', 'h': 'H', 'k': 'K', }.get(hdr['FILTER'], hdr['FILTER'])
