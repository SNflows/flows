"""
Load image also containes site data.
"""
...
class FlowsImage:
    ...
    @staticmethod
    def get_edge_mask(img_arr: np.ndarray, value: typing.Union[int, float, np.float64] = 0):
        """
        Create boolean mask of given value near edge of image.

        Parameters:
            img_arr (ndarray): image with values for masking.
...
def load_image(filename: str, target_coord: typing.Union[coords.SkyCoord, typing.Tuple[float, float]] = None):
    ...
    with fits.open(filename, mode='readonly') as hdul:
        ...
        while instrument_name is None:
            ...
            # NOT - ALFOSC
            elif telescope == 'NOT' and instrument in ('ALFOSC FASU', 'ALFOSC_FASU') and \
                    hdr.get('OBS_MODE', '').lower() == 'imaging':
...
def main():
    ...
    test_img = load_image(filename, (325.772590825, -17.54410925))
    print(test_img.peakmax)
    return test_img
