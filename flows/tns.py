...
def tns_search(coord: Optional[SkyCoord] = None, radius: u.Quantity = 3 * u.arcsec, objname: Optional[str] = None,
    ...
    search_data = [('api_key', (None, tnsconf['api-key'])), ('data', (None, json.dumps(json_file)))]
...
def tns_get_obj(name: str) -> Optional[dict]:
    ...
    get_data = [('api_key', (None, tnsconf['api-key'])), ('data', (None, json.dumps(params)))]
...
def tns_getnames(months: Optional[int] = None, date_begin: Optional[DateType] = None,
    ...
    params = {'discovered_period_value': months,  # Reported Within The Last
              'discovered_period_units': 'months', 'unclassified_at': 0,  # Limit to unclasssified ATs
              'classified_sne': 1,  # Limit to classified SNe
              'include_frb': 0,  # Include FRBs
              # 'name': ,
              'name_like': 0, 'isTNS_AT': 'all', 'public': 'all',  # 'ra':
              # 'decl':
              # 'radius':
              # 'coords_unit': 'arcsec',
              'reporting_groupid[]': 'null', 'groupid[]': 'null', 'classifier_groupid[]': 'null', 'objtype[]': objtype,
              'at_type[]': 'null', 'date_start[date]': date_begin.isoformat(), 'date_end[date]': date_end.isoformat(),
              # 'discovery_mag_min':
              # 'discovery_mag_max':
              # 'internal_name':
              # 'discoverer':
              # 'classifier':
              # 'spectra_count':
              'redshift_min': zmin, 'redshift_max': zmax,  # 'hostname':
              # 'ext_catid':
              # 'ra_range_min':
              # 'ra_range_max':
              # 'decl_range_min':
              # 'decl_range_max':
              'discovery_instrument[]': 'null', 'classification_instrument[]': 'null', 'associated_groups[]': 'null',
              # 'at_rep_remarks':
              # 'class_rep_remarks':
              # 'frb_repeat': 'all'
              # 'frb_repeater_of_objid':
              'frb_measured_redshift': 0,  # 'frb_dm_range_min':
              # 'frb_dm_range_max':
              # 'frb_rm_range_min':
              # 'frb_rm_range_max':
              # 'frb_snr_range_min':
              ...
              # 'frb_flux_range_min':
              # 'frb_flux_range_max':
              'num_page': 500, 'display[redshift]': 0, 'display[hostname]': 0, 'display[host_redshift]': 0,
              'display[source_group_name]': 0, 'display[classifying_source_group_name]': 0,
              'display[discovering_instrument_name]': 0, 'display[classifing_instrument_name]': 0,
              'display[programs_name]': 0, 'display[internal_name]': 0, 'display[isTNS_AT]': 0, 'display[public]': 0,
              'display[end_pop_period]': 0, 'display[spectra_count]': 0, 'display[discoverymag]': 0,
              'display[discmagfilter]': 0, 'display[discoverydate]': 0, 'display[discoverer]': 0, 'display[remarks]': 0,
              'display[sources]': 0, 'display[bibcode]': 0, 'display[ext_catalogs]': 0, 'format': 'csv'}
    ...
    text = str(con.text) + "\n"
    tab = Table.read(text, format='ascii.csv', guess=False, delimiter=',', quotechar='"', header_start=0, data_start=1)
