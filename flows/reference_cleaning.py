...
#--------------------------------------------------------------------------------------------------
def force_reject_g2d(xarray, yarray, image, get_fwhm=True, rsq_min=0.5, radius=10, fwhm_guess=6.0,
	fwhm_min=3.5, fwhm_max=18.0):
	"""
	...
	g2d.x_stddev.bounds = (
		fwhm_min * gaussian_fwhm_to_sigma,
...
#--------------------------------------------------------------------------------------------------
def clean_with_rsq_and_get_fwhm(masked_fwhms, masked_rsqs, references, min_fwhm_references=2,
	min_references=6, rsq_min=0.15):
	"""
	...
	if len(references) < 2:
		raise RuntimeError(f"{len(references)} References remaining; could not clean.")
...
def try_transform(source, target, pixeltol=2, nnearest=5, max_stars=50):
	...
	transform, (sourcestars, targetstars) = aa.find_transform(
		source,
		target,
...
def try_astroalign(source, target, pixeltol=2, nnearest=5, max_stars_n=50):
	...
	success = False
	try:
		source_stars, target_stars = try_transform(
			source,
			target,
			pixeltol=pixeltol,
			nnearest=nnearest,
...
#--------------------------------------------------------------------------------------------------
def min_to_max_astroalign(source, target, fwhm=5, fwhm_min=1, fwhm_max=4, knn_min=5,
	knn_max=20, max_stars=100, min_matches=3):
	...
	for max_stars_n in max_stars_list:
		for pixeltol in pixeltols:
			for nnearest in nearest_neighbors:
				source_ind, target_ind, success = try_astroalign(source, target,
					pixeltol=pixeltol,
					nnearest=nnearest,
...
#--------------------------------------------------------------------------------------------------
def get_new_wcs(extracted_ind, extracted_stars, clean_references, ref_ind, obstime, rakey='ra_obs',
	deckey='decl_obs'):

	targets = (extracted_stars[extracted_ind][:, 0], extracted_stars[extracted_ind][:, 1])

	c = SkyCoord(
		ra=clean_references[rakey][ref_ind],
		dec=clean_references[deckey][ref_ind],
		frame='icrs',
		obstime=obstime
	)
...
#--------------------------------------------------------------------------------------------------
def get_clean_references(references, masked_rsqs, min_references_ideal=6, min_references_abs=3,
	rsq_min=0.15, rsq_ideal=0.5, keep_max=100, rescue_bad: bool = True):
