...
def force_reject_g2d(xarray, yarray, image, get_fwhm=True, rsq_min=0.5, radius=10, fwhm_guess=6.0, fwhm_min=3.5,
					 fwhm_max=18.0):
	...
	for i, (x, y) in enumerate(zip(xarray, yarray)):
		...
		# Center
		xys[i] = np.array([gfit.x_mean + x - radius, gfit.y_mean + y - radius], dtype=np.float64)
		# Calculate rsq
...
def try_astroalign(source, target, pixeltol=2, nnearest=5, max_stars_n=50):
	...
	success = False
	try:
		source_stars, target_stars = try_transform(source, target,
												   pixeltol=pixeltol, nnearest=nnearest, max_stars=max_stars_n)
...
def min_to_max_astroalign(source, target, fwhm=5, fwhm_min=1, fwhm_max=4, knn_min=5, knn_max=20,
						  max_stars=100, min_matches=3):
	...
	for max_stars_n in max_stars_list:
		for pixeltol in pixeltols:
			for nnearest in nearest_neighbors:
				source_ind,target_ind,success = try_astroalign(source,
															  target,
															  pixeltol=pixeltol,
															  nnearest=nnearest,
...
def get_clean_references(references, masked_rsqs, min_references_ideal=6,
	...
	if np.sum(np.isfinite(masked_rsqs[mask])) >= min_references_ideal:
		if len(references[mask]) <= keep_max:
			...
			references[nmasked_rsqs[:keep_max]]
