...
def get_target(target):
    ...
    r = requests.get('https://flows.phys.au.dk/api/targets.php', params={'target': target},
...
def get_targets():
    ...
    r = requests.get('https://flows.phys.au.dk/api/targets.php', headers={'Authorization': 'Bearer ' + token})
    r.raise_for_status()
    jsn = r.json()
...
# --------------------------------------------------------------------------------------------------
def add_target(name, coord, redshift=None, redshift_error=None, discovery_date=None, discovery_mag=None,
               host_galaxy=None, ztf=None, sntype=None, status='candidate', project='flows'):
    """
    ...
    r = requests.post('https://flows.phys.au.dk/api/targets_add.php', data=params,
                      headers={'Authorization': 'Bearer ' + token})
    r.raise_for_status()
    jsn = r.json()
