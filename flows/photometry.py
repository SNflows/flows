...
def photometry(fileid, output_folder=None, attempt_imagematch=True, keep_diff_fixed=False,
	...
	for a, ax in ((0, ax3), (1, ax2)):
		...
		# for some reason
		profile_intp = UnivariateSpline(np.arange(0, len(profile)), profile - poffset,
			k=3, s=0, ext=3)
	...
	apphot_tbl = aperture_photometry(image.subclean, [apertures, annuli],
		mask=image.mask, error=image.error)
	...
	if datafile.get('diffimg') is not None:
		...
		diffimage = run_imagematch(datafile, target,
	...
	pars = bootstrap_outlier(x, y, yerr,
		n=nboot,
		model=model,
	...
	tab['pixel_column_psf_fit_error'].description = 'Error on measured location on image pixel columns from PSF photometry'
