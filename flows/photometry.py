...
def do_phot(fileid: int, cm_timeout: Optional[float] = None, make_plots: bool = True,
    ...
    for factor in [1,1.5,2,2.5,3,3.5,4,4.5,5,5.5]:
        # Trial smaller fitshape for better error.
        #factor = 1.5
        fitshape_small = int(factor*phot.fwhm)
        ...
        results_table_small.write(f"{directories.photometry_path[:-5]}_small_{factor}_{fitshape_small}.ecsv", format='ascii.ecsv', delimiter=',',
                                  overwrite=True)
